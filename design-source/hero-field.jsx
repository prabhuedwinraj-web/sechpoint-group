...
// point below the frame. Adapted from the shared Connectivity Field component,
// stripped of the timeline host and tweaks panel and driven by its own clock.
// The clock is interval-based on purpose: requestAnimationFrame is suspended in
// hidden/offscreen preview documents, which would leave the field unpainted.
...
function hexRgb(hex) {
  const h = String(hex || '#3ecfa4').replace('#', '');
...
function SechPointField(props) {
  const accent = props.accent || '#3ecfa4';
...
window.SechPointField = SechPointField;
